...
### CLI 경로 — Synology NAS (DSM FileStation)

NAS에서는 (a) 1회 자격증명 설정 → (b) 절대경로로 스캔 → (c) 평가 큐 처리.

```powershell
cd backend

# 1회만: 비밀번호를 OS 키체인에 보관
.\.venv\Scripts\python.exe -c "import keyring; keyring.set_password('PhotoArchiveEvaluator-NAS', 'USERNAME', 'PASSWORD'); print('saved')"

# 1회만: 로그인 검증 + 설정 DB 저장 (2FA 활성 시 OTP prompt)
.\.venv\Scripts\python.exe -m scripts.nas_login --url http://192.168.0.222:5000 --user USERNAME

# 이후 매 회: NAS 폴더 스캔
.\.venv\Scripts\python.exe -m scripts.scan_dsm --folder "/photo/My Pictures-2023"

# 평가 큐 처리 (로컬·NAS 모두 동일 큐, 워커가 nas_id 보고 다운로드 분기)
.\.venv\Scripts\python.exe -m scripts.process_eval_queue
```

> 2FA 사용자는 [DSM 보안 정책 메모](#2fa-주의)를 참조. 헤드리스 API용 **별도 비관리자 사용자**(File Station 권한만)를 만드는 것이 표준이며 권장됩니다.

### 웹 UI 경로

1. 헤더의 **로컬 스캔** 버튼 → 절대경로 입력 → 백그라운드 스캔 시작
2. 또는 **NAS 스캔** 버튼 → DSM 절대경로(예: `/photo/My Pictures-2023`) 입력 → 백그라운드 스캔
   - NAS가 미설정이면 안내 메시지 표시 (CLI `nas_login` 먼저 실행해야 함)
3. 헤더의 **평가 처리** 버튼 → 워커 백그라운드 실행 (큐의 모든 출처 처리)
4. 큐 통계 바가 5초마다 자동 갱신. 평가가 끝나면 **새로고침**으로 라이브러리에 반영

### 2FA 주의

`photo` 같은 일반 사용자 계정에 2FA가 켜져 있으면 헤드리스 동작이 어렵습니다 — DSM이 device_token을 잘 발급하지 않거나 시스템 정책에서 강제될 수 있음.

**권장 패턴**: DSM 제어판에서 별도 비관리자 사용자(예: `photo-api`)를 만들어 사진 폴더에 **읽기 전용** + **File Station 권한만** 부여. 2FA 비활성. 이렇게 하면 OTP 없이 자동 운영 가능하고 권한도 최소화됩니다.
